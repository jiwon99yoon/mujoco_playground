...
def main(argv):
  ...
  with mujoco.viewer.launch_passive(model, data,
    ...
    next_ctrl_t = data.time + ctrl_dt
    next_view_t = data.time + 1.0 / viewer_fps
    init = True

    while viewer.is_running():
        step_start = time.perf_counter()

        # --- 1) advance physics by exactly sim_dt ----------------------
        mujoco.mj_step(model, data)   # advances by model.opt.timestep == sim_dt

        # --- 2) run control exactly at ctrl_dt multiples ---------------
        if data.time + 1e-12 >= next_ctrl_t:
            # Build obs from *current MuJoCo sim state*
            ...
            # delta = np.asarray(action * eval_env._config.action_scale) * ctrl_dt
            # ctrl  = np.clip(data.ctrl + delta, np.asarray(eval_env._lowers), np.asarray(eval_env._uppers))
            # data.ctrl[:] = ctrl

            ctrl = data.ctrl.copy()

            # Arm (match training scaling; no extra dt here)
            delta = np.asarray(action[2:] * eval_env._config.action_scale)
            ctrl[2:] = np.clip(ctrl[2:] + delta, np.asarray(eval_env._lowers[2:]), np.asarray(eval_env._uppers[2:]))

            # Wheels with accel limit (use *ctrl_dt* for rate limiting)
            wheel_vel = husky_kinematics.IK(action[:2] * eval_env._config.mobile_action_scale, data.qvel[eval_env._robot_wheel_dofadr], ctrl_dt)
            ctrl[:2] = wheel_vel
        ...
        # --- 3) refresh viewer at viewer_fps ---------------------------
        if data.time + 1e-12 >= next_view_t:
            viewer.sync()
            next_view_t += 1.0 / viewer_fps

        # --- 4) handle manual reset ('R') ------------------------------
