...
# ==============================================================================
"""Bring a ball to a target and orientation."""
  ...
  config = config_dict.create(
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              # Gripper goes to the ball.
              gripper_ball=4.0,
              # Ball goes to the target mocap.
              ball_target=8.0,
...
class PandaPickBall(panda.PandaBase):
  """Bring a ball to a target. : changed from PandaPickBox - orientation 정보 주석 처리 """
  ...
  def __init__(
    ...
    xml_path = (
        ...
        / "franka_emika_panda"
        / "xmls"
        / "mjx_single_ball.xml"     # <---- 이부분 환경 xml 불러오기
    )
    super().__init__(
        xml_path,
        config,
  ...
  def reset(self, rng: jax.Array) -> State:
    rng, rng_ball, rng_target = jax.random.split(rng, 3)

    # intialize ball position
    ball_pos = (
        jax.random.uniform(
            rng_ball,
            (3,),
    ...
    info = {"rng": rng, "target_pos": target_pos, "reached_ball": 0.0}
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    out_of_bounds = jp.any(jp.abs(ball_pos) > 1.0)
  ...
  def _get_reward(self, data: mjx.Data, info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ball_target = 1 - jp.tanh(5 * pos_err)
    gripper_ball = 1 - jp.tanh(5 * jp.linalg.norm(ball_pos - gripper_pos))
    ...
    info["reached_ball"] = 1.0 * jp.maximum(
        info["reached_ball"],
        (jp.linalg.norm(ball_pos - gripper_pos) < 0.012),
    )

    rewards = {
        "gripper_ball": gripper_ball,
        "ball_target": ball_target * info["reached_ball"],
